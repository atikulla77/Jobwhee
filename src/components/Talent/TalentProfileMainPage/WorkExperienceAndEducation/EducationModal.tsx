...
interface EducationModalType {
    onEdit: () => void;
    onClose: () => void;
    experience?: Experience;
}

const EducationModal: React.FC<EducationModalType> = ({onEdit, onClose, experience}) => {
    ...
    useEffect(() => {
        if (experience) {
            ...
            setCurrentlyStudying(experience.isCurrent);
        ...
        try {
            if (isEditing && experience?.id) {
                await updateExperience(experience.id, {
                    ...
                    endDate: currentlyStudying ? undefined : `${endMonth} ${endYear}`,
    ...
    const toggleCheckbox = () => {
        setCurrentlyStudying(prev => !prev);
    };
    return (
        <>
            <div
            ...
            <div
                className="education-modal px-4 md:px-10 py-6 sm:mx-3 md:mx-0 max-w-[335px] md:max-w-[780px] lg:max-w-[876px] w-full fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-40 max-h-[90vh] bg-white rounded-[30px] overflow-y-auto">
                ...
                <div className=" flex gap-2 mt-[14px]">
                    <div className=" max-w-6">
                        <CheckBoxIcon checked={currentlyStudying} onToggle={toggleCheckbox}/>
                    </div>
                    <p className=" text-[#545454]">I currently study here</p>
                </div>
