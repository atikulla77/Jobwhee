...
const ClientProfileCard = ({
  ...
  return (
    ...
    >
      ...
      <h1 className="text-[20px] font-[600] mb-[10px]">{clientInfo?.name}</h1>
      ...
      <div className="w-[98px] h-[36px] flex items-center justify-center border-[1px] border-[#AEB3BC] rounded-[40px] mx-auto gap-[7px] md:mb-[22px] mb-[28px]">
        ...
        <p className="text-[18px] font-[500]">{clientInfo?.rating}</p>
      </div>
      <div className="flex justify-center items-center 2xl:gap-[28px] xl:gap-[12px] md:gap-[20px] gap-[12px]">
        <Circle title="Total Spent" value={clientInfo?.spends} />
        <Circle title="Total Jobs" value={clientInfo?.totalJobs} />
        <Circle title="Total Hours" value={clientInfo?.totalHours} />
      </div>
    </div>
  );
};
