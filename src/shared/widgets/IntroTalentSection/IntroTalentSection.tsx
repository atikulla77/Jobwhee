...
const IntroTalentSection = ({
    ...
    return (
        <div>
            ...
            >
                ...
                >
                    <div className={`min-h-[210px]`}>
                        <div className="relative  group flex items-center gap-2 mt-[20px] cursor-pointer">
                            <h2
                                className="xl:text-[25px] 2xl:text-[30px] font-medium leading-[100%] text-[#000000] pb-3"
                                style={{
                                    overflow: 'hidden',
                            ...
                            >
                                {user?.introTitle?.trim()
                                    ? user.introTitle.length > 40
                                        ? user.introTitle.slice(0, 40) + '...'
                                        : user.introTitle
                                    : "Your title"}
                            </h2>
                            <button
                                className={`${isViewedClient ? 'hidden' : 'block'} pb-3`}
                            ...
                            {user?.introTitle?.length > 40 &&
                                ...
                                !isVideoPopUpActive && (
                                    <div
                                        className="absolute bottom-0 left-1/2 -translate-x-1/2 w-auto min-w-[200px] bg-black text-white text-xs p-2 rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 max-w-[280px] z-[1]"
                                    >
                                        {user?.introTitle}
                                    </div>
                                )}
                        ...
                        <p
                            className="xl:text-[16px] 2xl:text-[18px] font-normal xl:leading-[20px] 2xl:leading-[32px]  text-[#545454] mt-[12px]">
                            {text?.trim() ? (
                                <>
                  ...
                  Your intro
                    ...
                    <div className={`flex flex-col gap-[11px]`}>
                        <div onClick={handleLanguageClick} className="flex gap-2 cursor-pointer items-center">
                            <h2 className={`text-[20px] font-bold text-[#000000]`}>Languages</h2>
                            <button className={`${isViewedClient ? 'hidden' : 'block'}`}><EditIcon/></button>

                        </div>
                        <div className="flex flex-wrap justify-start gap-[10px] 2xl:gap-[23px]">
                            {user?.languages && user?.languages.length > 0 ? (
                                user.languages.map((language: { language: string; proficiency: string }) => (
                                    <p key={language.language} className="flex">
                                        <span
                                            className="text-[16px] font-medium text-[#000000]">{language.language}:</span>
                      ...
                      {language.proficiency}
                                ...
                                <p className="text-[16px] font-normal text-gray-500">No languages mentioned.</p>
                            )}
                        </div>
                    </div>
                </div>
            ...
            <IntroProfileModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}/>

            <LanguageProfileModal isOpen={isLanguagePopUpActive} onClose={handleCloseLanguageModal}/>

            <VideoProfileModal isOpen={isVideoPopUpActive} onClose={handleVideoClick}/>
        </div>
    );
};
