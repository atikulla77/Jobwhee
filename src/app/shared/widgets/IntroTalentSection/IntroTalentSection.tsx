        ...
        >
          <div className={`min-h-[210px]`}>
            <div className={`flex gap-2 cursor-pointer items-center`}>
              <h2 className={`xl:text-[25px] 2xl:text-[30px] font-medium leading-[100%] text-[#000000]`}>
                {user?.introTitle?.trim() || "Your title"}
              </h2>
              <button
                className={`${isViewedClient ? " hidden cursor-pointer" : " block"}`}
              ...
              {user?.languages && user?.languages.length > 0 ? (
                user.languages.map((language: { language: string; proficiency: string }) => (
                  <p key={language.language} className="flex">
                    <span className="text-[16px] font-medium text-[#000000]">{language.language}:</span>
